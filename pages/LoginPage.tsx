  ...
  const resolveServerUrl = async (url: string) => {
    ...
    try {
      ...
      if (result && result.nextUrl) {
          ...
          // If the redirect URL doesn't have an API base, we might need to be careful.
          // Usually, resolveRedirect just follows the 302 location.
          // If it redirects to /api/..., we should strip that if we want the BASE url.
          // But here we are resolving the "Server Address".
          // If the user inputs "http://my-nas", and it redirects to "http://my-nas:5000/login",
          // then the base is likely "http://my-nas:5000".
          
          // Let's assume the redirect target IS the base or login page.
          // We'll take the origin of the redirect target as the new base.
          try {
            const nextUrlObj = new URL(result.nextUrl);
